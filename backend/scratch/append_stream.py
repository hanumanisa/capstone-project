...
with open('c:/xampp/htdocs/capstone-project/backend/api/ai_agent.py', 'r', encoding='utf-8') as f:
    code = f.read()

stream_func = '''

def execute_ai_query_stream(user, message: str, history: list = None):
    """
    Menjalankan kueri AI secara streaming menggunakan LangGraph React Agent.
    Mengembalikan generator yang menghasilkan chunk teks (SSE event-ready) dan metadata log di akhir.
    """
    if history is None:
        history = []
    ...
    try:
        ...
        # 4. Buat Instruksi Sistem (Prompt)
        base_instruction = (
        ...
        for chunk, metadata in agent.stream({"messages": messages}, stream_mode="messages"):
            ...
            if isinstance(chunk, AIMessage) or chunk.__class__.__name__ == 'AIMessageChunk':
                if hasattr(chunk, 'response_metadata') and chunk.response_metadata:
                    usage = chunk.response_metadata.get("token_usage") or {}
                    if usage:
                        tokens_used += usage.get("total_tokens") or 0

        # Output final payload marker to process logs
        yield json.dumps({
            "done": True, 
        ...
        print("Agent Execution Stream Error:", str(e))
...
if 'def execute_ai_query_stream' not in code:
    code += stream_func
    with open('c:/xampp/htdocs/capstone-project/backend/api/ai_agent.py', 'w', encoding='utf-8') as f:
        f.write(code)
    print('execute_ai_query_stream appended successfully.')
else:
    print('execute_ai_query_stream already exists.')
