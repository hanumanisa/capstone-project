...
const CoursePage = () => {
    ...
    const openEdit = (item) => {
        ...
        setFormData({
            ...
            original_course_name: item.course_name || '',
        });
        setShowModal(true);
    };
    ...
    const handleSave = async () => {
        ...
        const normId = norm(formData.course_id);
        const normName = norm(formData.course_name);
        ...
        if (!isEdit) {
            ...
            const isDuplicate = courses.some(c => {
                const existingId = norm(c.course_id);
                const existingName = norm(c.course_name);
            ...
            if (isDuplicate) {
                setToast({ message: "course already exist, input other course code and course name", type: 'error' });
                return;
            }
        } else {
            // Edit mode: only validate name duplicate if user actually changed course_name
            const normOriginalName = norm(formData.original_course_name);
            if (normName && normName !== normOriginalName) {
                const isDuplicate = courses.some(c => {
                    if (norm(c.course_id) === norm(formData.course_id)) {
                        return false;
                    }
                    const existingName = norm(c.course_name);
        ...
        try {
            ...
            let errorMsg = isEdit ? 'Failed to update course' : 'Failed to add course';
            if (err.response?.data) {
                const data = err.response.data;
                if (data.course_id) {
                    errorMsg = Array.isArray(data.course_id) ? data.course_id[0] : data.course_id;
                } else if (data.course_name) {
                    errorMsg = Array.isArray(data.course_name) ? data.course_name[0] : data.course_name;
            ...
            if (typeof errorMsg === 'string' && errorMsg.includes("course already exist")) {
                errorMsg = "course already exist, input other course code and course name";
            }
    ...
    const handleExport = () => {
        ...
        XLSX.utils.sheet_add_aoa(ws, [
            ["[1] PT Sarana Multi Infrastruktur (Persero), “Course Data,” Internal Company Database, 2026."]
                    ...
                    <div className="flex items-center space-x-2">
                        {canReport && (
                            <button
                                ...
                                className={`w-36 py-2 rounded-lg font-medium text-sm transition-all shadow-sm outline-none border-none ${courses.length === 0 ? 'bg-gray-400 text-gray-200 cursor-not-allowed' : 'bg-[#2174C3] hover:bg-[#1A5E9D] text-white cursor-pointer'}`}
                            >
                                Download Report
                            </button>
                        )}
